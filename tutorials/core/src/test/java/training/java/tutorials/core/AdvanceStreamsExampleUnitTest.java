...
import training.java.tutorials.core.streams.Category;
...
public class AdvanceStreamsExampleUnitTest {
  ...
  public void reducingStreamUnitTest() {
    ...
    Optional<BigDecimal> opt = products.stream().map(Product::getPrice)
      .reduce((result, element) -> result.add(element)); // Can also be written with a method reference: BigDecimal::add

    opt.ifPresentOrElse(total -> System.out.printf("The total value of all products is: $ %.2f%n", total),
    ...
    // The third parameter is a combiner function to combine intermediate results; this is useful for example for a
    // parallel
  ...
  public void totalPerCategoryProductUnitTest() {
    ...
    Map<Category, BigDecimal> totalPerCategory1 = products.stream()
      .collect(Collectors.toMap(Product::getCategory,   // Key mapper function
        Product::getPrice,      // Value mapper function
        BigDecimal::add));      // Merge function
    System.out.println(totalPerCategory1);

    Map<Category, BigDecimal> totalPerCategory2 = products.stream().collect(Collectors.groupingBy(Product::getCategory,
  ...
  public void groupByProductUnitTest() {
    ...
    // Mapping products to product names with map(...) won't work, because the result of the map(...) operation is a
    // stream of strings.
    // The information about the categories will have been thrown away, so you can't group on categories anymore in
    // the collector.
    ...
    // Note that Collectors.mapping(...) takes a mapping function and another downstream collector to determine how
  ...
  public void specializedStreamsExampleUnitTest() {
    List<Product> products = ExampleData.getProducts();
    // Compared to Stream<Double>, this avoids boxing and unboxing primitive double values into and out of Double
    // wrapper objects.
    DoubleStream prices = products.stream().mapToDouble(product -> product.getPrice().doubleValue());
    ...
    DoubleSummaryStatistics statistics = products.stream().mapToDouble(product -> product.getPrice().doubleValue())
      .summaryStatistics();
