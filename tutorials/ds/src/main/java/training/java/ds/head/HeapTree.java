...
public class HeapTree<T> {
  ...
  /**
   * Function to check whether a node is a leaf or internal node, use to check a node in a heap tree
   *
   * @param pos The index of the cell in the array
   **/
  ...
  /**
   * This function will shiftdown a node until  it is a leaf node or it is smaller then its both right and left child //
   *   * @param keys the heap tree store in an array //   * @param n frequency of node in the array //   * @param pos
  ...
  public void shiftDown(int pos) {
    ...
    while(!isLeaf(pos)) {
      int j = leftChild(pos);
      if((j < size - 1) && (comparator.compare(node[j], node[j + 1]) > 0)) {
        j++;
      }
      if(comparator.compare(node[pos], node[j]) < 0) {
        return;
      }
      swap(pos, j);
      pos = j;
  ...
  private void shiftUp(int pos) {
    while(pos > 0) {
      ...
      if(comparator.compare(node[pos], node[parent]) > 0) {
        return;
      }
      swap(parent, pos);
      pos = parent;
